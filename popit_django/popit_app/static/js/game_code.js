...
Game.prototype.updateGame = function(){
  ...
  for(var i = 0; i < this.balloonsArray.length; i++){
    ...
    var posy = parseInt(this.balloonsArray[i].el.style.bottom.replace('px', ''));
    var posx = parseInt(this.balloonsArray[i].el.style.left.replace('px', ''));
    this.hitbox = [[posx, posx+53],[posy, posy+53]];
    console.log("PosBal :", this.hitbox);
    var mousePos = findScreenCoords(mouseEvent);
    console.log("PosMouse :", mousePos);
  }
      
  // La souris doit taper dans la hitbox
};
